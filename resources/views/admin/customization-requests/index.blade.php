...
<div class="mb-6 flex justify-between items-center">
    <h1 class="text-3xl font-bold font-space-mono text-gray-800">Customization Requests</h1>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($requests as $request)
                    <tr class="hover:bg-white/40 transition-colors cursor-pointer" onclick="window.location='{{ route('admin.customization-requests.show', $request->id) }}'">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 truncate max-w-xs">{{ Str::limit($request->details, 50) }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" onclick="event.stopPropagation()">
                            <a href="{{ route('admin.customization-requests.show', $request->id) }}" class="text-indigo-600 hover:text-indigo-900 font-bold mr-4">View Details</a>
                            <form action="{{ route('admin.customization-requests.update', $request->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('PUT')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-gray-500 italic">No requests found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
