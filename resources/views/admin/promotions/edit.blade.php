...
<div class="mb-6 flex justify-between items-center">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-3xl font-bold font-space-mono text-gray-800">Edit Promotion</h1>
    </div>
</div>

<div class="glass-panel p-8 max-w-2xl" x-data="{ type: '{{ old('type', $promotion->type) }}' }">
    <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST">
        ...
        <div class="space-y-6">
                ...
                <div x-show="type === 'category'" x-cloak>
                    ...
                    <select name="category_ids[]" id="category_ids" multiple
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ in_array($category->id, old('target_ids', is_array($promotion->target_ids) ? $promotion->target_ids : [])) ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div x-show="type === 'product'" x-cloak>
                    ...
                    <select name="product_ids[]" id="product_ids" multiple
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ in_array($product->id, old('target_ids', is_array($promotion->target_ids) ? $promotion->target_ids : [])) ? 'selected' : '' }}>
                                {{ $product->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="discount_percentage" id="discount_percentage" value="{{ old('discount_percentage', $promotion->discount_percentage) }}" required 
                       class="w-full bg-white/50 border border-gray-200 rounded-sm px-4 py-3 focus:outline-none focus:border-black transition-colors font-instrument"
                       placeholder="e.g. 15">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="start_date" id="start_date" value="{{ old('start_date', $promotion->start_date->format('Y-m-d\TH:i')) }}" required
                           class="w-full bg-white/50 border border-gray-200 rounded-sm px-4 py-3 focus:outline-none focus:border-black transition-colors font-instrument">
                    @error('start_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" name="end_date" id="end_date" value="{{ old('end_date', $promotion->end_date->format('Y-m-d\TH:i')) }}" required
                           class="w-full bg-white/50 border border-gray-200 rounded-sm px-4 py-3 focus:outline-none focus:border-black transition-colors font-instrument">
                    @error('end_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
